...
import { VidhyaVanamGradeContext } from "./VidhyaVanamGradeContext";
...
const VidhyaVanamGrade = () => {
  const { contextData } = useContext(VidhyaVanamGradeContext);
  ...
  const fetchData = async () => {
    const res = await getRequest("/vidhyavanam/grade-levels");
    if (res.success) {
  ...
  const onSubmit = async () => {
    ...
    try {
      let res;
      if (editId !== null) {
        res = await putRequest(`/vidhyavanam/grade-levels/${editId}`, payload);
        if (res.success) {
        ...
        res = await postFormData("/vidhyavanam/grade-levels", payload);
        if (res.success) {
...
export default VidhyaVanamGrade;
