...
const [confirmModal, setConfirmModal] = useState(false);
  ...
  const fetchData = async () => {
    setLoading(true)
    const res = await getRequest("/vidhyavanam/contact-info");
    if (res.success) {
      setData(res.data);
    } else {
    }
    ...
    try {
      if (editId !== null) {
        const res = await putRequest(
          `/vidhyavanam/contact-info/${editId}`,
          formData
        );
        if (res.success) {
        ...
        const res = await postRequest("/vidhyavanam/contact-info", formData);
        if (res.success) {
  ...
  const onDeleteClick = async (id) => {
    const res = await deleteRequest(`/vidhyavanam/contact-info/${id}`);
    if (res.success) {
          ...
          <ModalBody className='text-center'>
            ...
            <div className='d-flex justify-content-center gap-2 mt-4'>
              <Button
                ...
                className='p-3'
                onClick={async () => {
                  const res = await deleteRequest(`/vidhyavanam/contact-info/${deleteId}`);
                  if (res.success) {
                    toast.success("Deleted successfully");
                    fetchData();
                  } else {
...
export default VidhyaVanamContact;
