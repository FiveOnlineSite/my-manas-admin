...
const [confirmModal, setConfirmModal] = useState(false);
  ...
  const onSubmit = async () => {
    ...
    let res;
    if (editId) {
      res = await putRequest(`/academy/achievements/${editId}`, payload);
    } else {
      res = await postFormData("/academy/achievements", payload);
    }
  ...
  const onDeleteClick = async (id) => {
    const res = await deleteRequest(`/academy/achievements/${id}`);
    if (res.success) {
      fetchData();
          ...
          <ModalBody className='text-center'>
            ...
            <div className='d-flex justify-content-center gap-2 mt-4'>
              <Button
                ...
                className='p-3'
                onClick={async () => {
                  const res = await deleteRequest(`/academy/achievements/${deleteId}`);
                  if (res.success) {
                    toast.success("Deleted successfully");
                    fetchData();
                  } else {
